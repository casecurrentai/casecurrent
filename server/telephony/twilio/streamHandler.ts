...
const TOKEN_MAX_AGE_SECONDS = 120;
...
export function generateStreamToken(secret: string, callSid: string, timestamp: number): string {
  return createHmac("sha256", secret)
    .update(`${callSid}.${timestamp}`)
    .digest("hex");
}

export function verifyStreamToken(secret: string, callSid: string, timestamp: number, token: string): boolean {
...
export function handleTwilioMediaStream(twilioWs: WebSocket, req: IncomingMessage): void {
  ...
  if (secret && tsParam && tokenParam) {
    ...
    console.log(`[TwilioStream] [${requestId}] No STREAM_TOKEN_SECRET configured - allowing without auth (dev mode)`);
  }
...
Be professional, empathetic, and concise. Do not provide legal advice.`,
    ...
    if (pendingTwilioAudio.length > 0) {
      console.log(`[TwilioStream] [${requestId}] Flushing ${pendingTwilioAudio.length} buffered audio frames`);
    ...
    try {
      ...
      if (message.type === "session.created") {
        ...
        if (message.session?.input_audio_format) {
          console.log(`[TwilioStream] [${requestId}] Confirmed input format: ${message.session.input_audio_format}`);
        }
        if (message.session?.output_audio_format) {
          console.log(`[TwilioStream] [${requestId}] Confirmed output format: ${message.session.output_audio_format}`);
        }
        ...
        if (audioBase64 && streamSid && twilioWs.readyState === WebSocket.OPEN) {
          ...
          if (openAiFrameCount % LOG_FRAME_INTERVAL === 0) {
            console.log(`[TwilioStream] [${requestId}] Sent ${openAiFrameCount} audio frames to Twilio`);
          }
        }
        ...
        console.log(`[TwilioStream] [${requestId}] AI response transcript: ${message.transcript?.substring(0, 100)}...`);
      } else if (message.type === "conversation.item.input_audio_transcription.completed") {
        console.log(`[TwilioStream] [${requestId}] User said: ${message.transcript?.substring(0, 100)}...`);
  ...
  twilioWs.on("message", (data) => {
    try {
      ...
      if (message.event === "connected") {
        ...
        console.log(`[TwilioStream] [${requestId}] Twilio stream started: streamSid=${streamSid} callSid=${callSid}`);
        if (message.start?.mediaFormat) {
          const mf = message.start.mediaFormat;
          console.log(`[TwilioStream] [${requestId}] Media format: encoding=${mf.encoding} sampleRate=${mf.sampleRate} channels=${mf.channels}`);
        }
        ...
        if (audioPayload) {
          ...
          if (twilioFrameCount % LOG_FRAME_INTERVAL === 0) {
            console.log(`[TwilioStream] [${requestId}] Received ${twilioFrameCount} audio frames from Twilio`);
          }
  ...
  twilioWs.on("close", (code, reason) => {
    ...
    console.log(`[TwilioStream] [${requestId}] Final stats: twilioFrames=${twilioFrameCount} openAiFrames=${openAiFrameCount}`);
    if (openAiWs && openAiWs.readyState === WebSocket.OPEN) {
